...
public class Planner {
	...
	public static void createModule(Scanner sIn) {
		...
		while (!done) {
			...
			switch (choice) {
				case "1":
					addPlan(sIn, "Packing");
					break; //fixed wrong comment in debug line
				case "2":
	...
	public static void addPlan(Scanner sIn, String planType) {
		...
		if (planType.equalsIgnoreCase("Packing")) {
			...
			PackingPlan packingPlan = new PackingPlan(planTitle, destination, travelers, planType);
			...
			System.out.printf("\nThe '%s' Shopping Plan has been created and is ready for update.\n", shoppingPlan.getTitle());//Fixed method name of object to display
	...
	public static void changeItemQty(Scanner sIn, ToDo plan) {
		...
		if (changeQty < 0 && Math.abs(changeQty) >= plan.getAmount(item)) { plan.delete(item); }
		else { plan.change(item, changeQty); }
